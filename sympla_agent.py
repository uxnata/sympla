#!/usr/bin/env python3
"""
Sympla kids/family events — агент сбора (скелет под Claude Code).

Архитектура (5 стадий, см. матрицу доступа):
  1) discovery   — собрать список URL событий (детерминированно)
  2) fetch+parse — по каждому URL вытащить JSON-LD (schema.org/Event), HTML — фоллбэк
  3) enrich(LLM) — доразметить поля, которых нет в источнике (возраст, дети/родители, язык, формат, категория)
  4) normalize   — привести к целевой плоской схеме, вычислить is_free / status / updated_at
  5) validate    — ссылка открывается? дата валидна? обяз. поля на месте? иначе status="требует проверки"

Принцип: ФАКТЫ приносит только детерминированный код (стадии 1,2). LLM (стадия 3)
классифицирует ПОВЕРХ уже добытого текста и НЕ выдумывает факты. Стадия 5 ловит мусор.

ЧТО НУЖНО ПОДТВЕРДИТЬ НА ЖИВОЙ СТРАНИЦЕ (помечено TODO[live]):
  - discovery-эндпоинт листинга (см. CONFIG.LISTING)
  - HTML-фоллбэк-селекторы, если JSON-LD неполный
"""
...
if __name__ == "__main__":
    ...
    # Боевой запуск (нужен список URL и сеть к Sympla):
    #   run(urls=[...], limit=100)
    # либо после реализации discovery:
    #   run(limit=100)
    print(__doc__)
